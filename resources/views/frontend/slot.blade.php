...
@section('content')
    ...
    <div id="game-cards" class="grid grid-cols-3 md:grid-cols-3 lg:grid-cols-6 gap-6 px-6 pb-6">
        @foreach ($games as $game)
            <div class="game-card"
                onclick="openModal('{{ $game->game_name }}', '{{ $game->game_provider_code }}', '{{ $game->game_image }}', '{{ route('game.playGame', $game->id) }}')">
    ...
    <button id="load-more" onclick="loadMoreGames()" class="mt-4 w-full bg-gray-600 text-white py-2 rounded-md">Load
        More</button>
